package
{
	...
	public class FrameAnimator extends Bitmap
	{
		private var _node:XML;
		private var _img:BitmapData;
		private var _loop:Boolean;
		public function FrameAnimator(img:BitmapData, node:XML, loop:Boolean = true)
		{
			super();

			this._img = img;
			this.node = node;  // call the setter
			this._loop = loop;
		}
		...
		public function onFrame(frame:uint):void
		{
			if (_frames > 1)
			{
				if ((frame % _frameLength) == 0)
				{
					...
					if (!_loop && _frame == (_frames - 1))
					{
						//
						// Stop looping;  arguably we should throw an event here
						_frames = 1;
					}
				}
			}
